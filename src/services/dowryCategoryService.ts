...
import { getUserDowryItems } from "@/services/dowryItemService";
import type {
  DowryCategoryItem,
...
export async function getDowryCategories(): Promise<DowryCategoryItem[]> {
  ...
  if (countError) {
    ...
    ((countData ?? []) as DowryCategoryItemCountRow[]).forEach((item) => {
      countsByCategorySlug.set(item.category_slug, {
        total: toSafeNumber(item.total_count),
        completed: toSafeNumber(item.completed_count),
      });
    });
  }
  ...
  await Promise.all(
    categories.map(async (category) => {
      try {
        const items = await getUserDowryItems(category.slug);
        ...
        expenseByCategoryId.set(category.dbId, expense);
      } catch (error) {
        console.log("Dowry shared item summary error:", error);
        expenseByCategoryId.set(category.dbId, 0);
      }
    }),
  );
  ...
  const { data: budgetData, error: budgetError } = await supabase.rpc(
    "get_user_dowry_category_budgets",
  );

  if (budgetError) {
    console.log("Dowry budget rpc error:", budgetError);
  } else {
    ((budgetData ?? []) as UserDowryCategoryBudgetTableRow[])
      .filter((item) => categoryDbIds.includes(item.category_id))
      .forEach((item) => {
